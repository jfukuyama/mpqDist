my_plot_tree = function (physeq, method = "sampledodge", nodelabf = NULL, color = NULL, 
    ...
    branch.colors = NULL) 
{
    ...
    treeDT <- treeSegs$edgeDT
    if (!is.null(branch.colors)) {
        requireNamespace("ape")
        phy <- phy_tree(physeq)
        edge_color_map <- rep("gray", nrow(treeDT))
        
        ## Convert to edge matrix
        edge.mat <- phy$edge
        node2edge <- split(seq_len(nrow(edge.mat)), edge.mat[,1])  # parent -> rows
        
        ## Helper: find all edges from MRCA to each tip in group
        get_descendant_edges <- function(phy, tips) {
            mrca <- ape::getMRCA(phy, tips)
            if (is.null(mrca)) return(integer(0))
            all.descendants <- phangorn::Descendants(phy, mrca, type = "all")
            edges <- which(treeDT$V1 %in% c(mrca, all.descendants))
            return(edges)
        }

        group.colors <- names(branch.colors)
        color.palette <- scales::hue_pal()(length(group.colors))  # assign distinct colors

        for (i in seq_along(group.colors)) {
            group <- group.colors[i]
            tips <- branch.colors[[group]]
            edge.idx <- get_descendant_edges(phy, tips)
            color <- color.palette[i]
            edge_color_map[edge.idx] <- color
        }
        treeDT[, branch_color := edge_color_map]
    } else {
        treeDT[, branch_color := NA_character_]
    }

    ## Replace the call to geom_segment() for horizontal edges:
    p = ggplot(data = treeDT) + 
        geom_segment(aes(x = xleft, xend = xright, y = y, yend = y, color = branch_color), na.rm = TRUE) +
        geom_segment(aes(x = x, xend = x, y = vmin, yend = vmax), color = "gray", data = treeSegs$vertDT, na.rm = TRUE) +
        scale_color_identity()  # so we use raw colors in branch_color
